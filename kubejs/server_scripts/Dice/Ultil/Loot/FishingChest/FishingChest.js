...
// import { lootbigFailure as RewardEvents.lootbigFailure, lootbigSuccess as RewardEvents.lootbigSuccess } from "../RewardEvents"
// import { randomChance as Utils.randomChance } from "../../../../API/Utils"
...
const FishingChest = {
     ...
     * @param {double} dz
     */
    fishingChest: function (player, hook, dx, dy, dz, event) {
        const {
            level
        } = player
        const {
     ...
     */
    trackingFallingBlock: function (entity) {
        const {
            block: { pos },
            server,
            level
     ...
     */
    fishingChestLoot: function (event) {
        const {
            level,
            player,
            block,
        ...
        let skill = Pmmo.getSkillLevel("wisdom", player) + (Pmmo.getSkillLevel("dexterity", player) / 2 | 0)
        let lootTable
     ...
     * @param {$Block} block
     */
    popItem: function (lootTable, block) {
        lootTable.forEach(
            item => {
                block.popItem(item)
            }
...
BlockEvents.rightClicked(event => {
    ...
    if (block.id == "minecraft:barrel") {
        if (entity.persistentData.getBoolean('fallingblock') == true) {
            FishingChest.fishingChestLoot(event)
